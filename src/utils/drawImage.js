...
try { var options = Object.defineProperty({}, "passive", { get: function () { passiveSupported = !0; } }); window && window.addEventListener("test", null, options); } catch (err) { }
...
export default function DrawImage(app, opts){
  ...
  let getPosition = (e) => {
    ...
    if (e.type.indexOf('touch') !== -1) { // Touch event
  ...
  let draw = () => {
    if(drawing){
      if(drawingType == 'line') {
        ...
        context.clearRect(_x - _r - lw, _y - _r - lw, _r * 2 + (lw*2), _r * 2 + (lw*2));
        context.beginPath();
  ...
  let getImageData = () => context.getImageData(0, 0, canvas.width, canvas.height)
    ...
    switch(type){
      case 'change':
        app.fire('change', {
          canUndo : canUndo(),
          canRedo : canRedo(),
          canvasImage: canvas.toDataURL(opts.mimeType)
        });
      break;
    }
  }
  ...
  this.syncImage = (imgData) => {
    var curUrl = canvas.toDataURL(opts.mimeType);
    if(curUrl != imgData){
      _prevState = getImageData();
